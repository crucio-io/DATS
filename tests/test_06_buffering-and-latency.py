...
class ImpairDelay(dats.test.binsearch.BinarySearch):
    ...
    def run_test(self, pkt_size, duration, value):
        ...
        rx, tx, _ = self._tester.tot_stats()
        ierrors, _ = self._tester.tot_ierrors()

        rx_total = rx
        tx_total = tx

        can_be_lost = int(tx_total * float(config.getOption('toleratedLoss')) / 100.0)
        logging.verbose("RX: %d (ierrors: %d); TX: %d; dropped: %d (tolerated: %d)",
                        rx_total, ierrors, tx_total, tx_total - rx_total, can_be_lost)
