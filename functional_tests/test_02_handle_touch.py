...
class HandleTouchTest(dats.test.passfail.PassFail):
    """Functional tests of the l2fwd mode

    This test suite checks the correctness of the ``l2fwd`` mode in
    PROX.
    """
    ...
    def setup_class(self):
        ...
        """
        self._tester = self.get_remote('tester').run_prox_with_config("02_handle_touch-gen-4.cfg", "-e -t")
        self._sut = self.get_remote('sut').run_prox_with_config("02_handle_touch-sut-4.cfg", "-t")
        self._cores = [1, 2, 3, 4]
    ...
    def Forwarding(self):
        ...
        # the SUT (s_tx).
        core_map = {
                1:{'t_rx':2, 's_rx':1, 's_tx':1, 't_mac':[0x55, 0x55, 0x55, 0x00, 0x00, 0x02], 's_mac':[0x77, 0x77, 0x77, 0x00, 0x00, 0x01]},
                2:{'t_rx':1, 's_rx':2, 's_tx':2, 't_mac':[0x55, 0x55, 0x55, 0x00, 0x00, 0x01], 's_mac':[0x77, 0x77, 0x77, 0x00, 0x00, 0x02]},
                3:{'t_rx':4, 's_rx':3, 's_tx':3, 't_mac':[0x55, 0x55, 0x55, 0x00, 0x00, 0x04], 's_mac':[0x77, 0x77, 0x77, 0x00, 0x00, 0x03]},
                4:{'t_rx':3, 's_rx':4, 's_tx':4, 't_mac':[0x55, 0x55, 0x55, 0x00, 0x00, 0x03], 's_mac':[0x77, 0x77, 0x77, 0x00, 0x00, 0x04]},
        }
        ...
        for tx_core in sorted(self._cores):
            ...
            self.cmp(payload[ 0: 6], core_map[tx_core]['s_mac'], "    ... and dst MAC must be correct")
            self.cmp(payload[ 6:12], core_map[tx_core]['t_mac'], "    ... and src MAC must be correct")
